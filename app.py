...
def signup_page():
    return render_template('2_signup.html')
...
def check_duplicate():
    try:
        ...
        sqlite_cursor.execute(
            "SELECT COUNT(*) FROM users WHERE id = ?", (id_to_check,))
...
def sign_up():
    try:
        ...
        # 데이터베이스에 저장
        sqlite_cursor.execute(
            "INSERT INTO users (id, password, gender, age) VALUES (?, ?, ?, ?)", (id, password, gender, age))
        sqlite_conn.commit()
...
def login():
    ...
    if result:
        return redirect(url_for('select_period'))
    else:
        flash('로그인 실패.')
        return render_template('1_login.html')
...
def select_period():
    ...
    changing_date = session.get('changing_date')
    return render_template('3_select_period.html', start_date=start_date, end_date=end_date,  changing_date=changing_date, start_date2=start_date2)


@app.route('/information')
def information():
    return render_template('4_information.html')
...
def save_survey_data_to_main():
    try:
        ...
        # 응답을 JSON 형태로 보냄
        return jsonify({'status': 'success', 'survey_data': session['survey_data']})
...
def save_survey_data():
    try:
        ...
        # 세션에 데이터 저장
        session['survey_data'] = {
            'age': age, 'relationship': relationship, 'gender': gender}

        # 데이터를 문자열로 만듦
        result_string = f'나이: {age}, 관계: {relationship}, 성별: {gender}'

        return render_template('5_select_location.html', result_string=result_string, start_date=start_date)
...
def select_location():
    ...
    return render_template('5_select_location.html', survey_data=survey_data, datecount=datecount, changing_date=changing_date)
...
def update_date():
    ...
    changing_date = data.get('start_date')
    changing_date = datetime.strptime(changing_date, '%Y-%m-%d')
    changing_date = changing_date + timedelta(days=1)
    session['changing_date'] = changing_date
    return jsonify({'success': True, 'changing_date': changing_date})
...
def set_attraction():
    ...
    updated_attractions = list(
        set(current_attractions) - set(recommended_attractions))
...
def get_recommendations_for_main(user_gender, user_age, user_relationship, user_destination):
    ...
    for attraction_id in df_sqlite['관광지'].unique():
        ...
        prediction = model.predict(
            user_id, attraction_id, r_ui=None, clip=False)

        user_recommendations.append(
            {'관광지': attraction_id, '예상평점': prediction.est})

    user_recommendations = sorted(
        user_recommendations, key=lambda x: x['예상평점'], reverse=True)[:3]
...
def get_recommendations():
    ...
    # 함수 호출 및 결과 받기
    recommendations = get_recommendations_for_main(
        user_gender, user_age, user_relationship, user_destination)
    attraction_names = [recommendation['관광지']
                        for recommendation in recommendations]
    return jsonify({'status': 'success', 'attraction_names': attraction_names})


@app.route('/select_sleep')
def sleep():
    return render_template('6_select_sleep.html')
...
def nearest_neighbor_tsp(coords):
    ...
    while unvisited:
        nearest_city = min(unvisited, key=lambda city: haversine_distance(
            coords[current_city], coords[city]))
...
def process_selected_attractions():
    ...
    for attraction in selectedAttractions:
        ...
        geocode_result = gmaps.geocode(
            '%s,%s' % (attraction, selected_location_name))
...
@app.route('/select_move')
def move():
    return render_template('7_select_move.html')


@app.route('/select_move_last')
def move_last():
    return render_template('8_select_move_last.html')
...
def table():
    ...
    return render_template('9_result_table.html', dayAttractions=dayAttractions, daylocated=daylocated, daymode=daymode)
